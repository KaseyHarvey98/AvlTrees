...
{
        if ( t == NULL )
            // can't delete from an empty tree return;
            if ( x < t->element ) {
                ...
                // delete from the right subtree remove( x, t->right );
                if ( height( t->left ) - height( t->right ) == 2 ) {// unbalanced // left subtree too tall
